...
- [ ] **T007** Добавить неизменяемую модель результата одной попытки с длительностью и числом фактически полученных bytes. [FR-007]
- [ ] **T008** Добавить неизменяемую итоговую модель для request count, total duration, average duration, total bytes, MB, MB/s и Mbps. [FR-008] [FR-009] [FR-010] [FR-011]
- [ ] **T009** Реализовать проверку ровно 10 результатов и положительной длительности каждой попытки до агрегации. [FR-003] [ERR-005]
- [ ] **T010** Реализовать расчёт total duration, среднего времени, total bytes и десятичных MB без промежуточного округления. [FR-008] [FR-009]
- [ ] **T011** Реализовать MB/s и Mbps из total bytes / total duration, не усредняя индивидуальные скорости. [FR-010] [FR-011]
- [ ] **T012** Покрыть модели, формулы, инварианты, нулевой объём и неодинаковые длительности unit-тестами. [FR-003] [FR-008] [FR-009] [FR-010] [FR-011] [ERR-005]

## Этап 3. HTTP measurement

- [ ] **T013** Добавить одну минимальную прикладную ошибку измерения для передачи ожидаемой причины в CLI без traceback. [ERR-002] [ERR-003] [ERR-004] [ERR-005]
- [ ] **T014** Реализовать один синхронный логический GET через переданный `httpx.Client`, полный streaming final body через `iter_raw()` и прямую сумму chunk lengths. [FR-003] [FR-005] [FR-007]
- [ ] **T015** Ограничить timing границами непосредственно перед request и после полного body через `time.perf_counter()`, отклоняя длительность `<= 0`. [FR-006] [ERR-005]
- [ ] **T016** После полного чтения final body обрабатывать только конечные статусы 4xx/5xx как HTTP error. [ERR-002]
- [ ] **T017** Добавить создание одного синхронного client с `follow_redirects=True`, `max_redirects=20` и явным `httpx.Timeout(30.0)`. [FR-014] [ERR-003] [NFR-002]
- [ ] **T018** Проверить redirect chain, полное чтение промежуточных body, исключение их bytes, unsupported scheme, cycle и redirect limit. [FR-004] [FR-005] [FR-014] [ERR-004]
- [ ] **T019** Покрыть measurement детерминированными `MockTransport`/`SyncByteStream` tests без реального интернета и `sleep`. [FR-005] [FR-006] [FR-007] [NFR-003]
...
- [ ] **T022** Реализовать строго последовательный цикл из 10 попыток через один context-managed client без request body и retries. [FR-003] [FR-004] [NFR-002]
